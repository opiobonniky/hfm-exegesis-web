...
const PublicLayout = () => {
  ...
  return (
    <div className="w-full bg-background text-foreground overflow-x-hidden">
      ...
      {/* NAV */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-background/95 backdrop-blur-md border-b border-border" : "bg-background/95 backdrop-blur-md border-b border-border"}`}>
        <div className="w-full px-4 sm:px-6 lg:px-12">
          <div className="flex items-center justify-between h-14 sm:h-16 lg:h-20">
              ...
              {menuItems.map((item) =>
                item.subItems ? (
                  <div key={item.label} className="relative group">
                    <button className="px-3 py-2 font-black rounded-xl transition-all whitespace-nowrap uppercase tracking-widest active:scale-95 text-[10px] text-muted-foreground hover:text-primary hover:bg-muted flex items-center gap-1">
                      {item.label}
                    ...
                    <div className="absolute top-full left-1/2 -translate-x-1/2 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 pointer-events-none group-hover:pointer-events-auto">
                      <div className="bg-card rounded-2xl shadow-2xl border border-border py-2 min-w-[180px] overflow-hidden">
                        {item.subItems.map((sub) => (
                          <button key={sub.label} onClick={() => handleMenuClick(sub.href)} className="w-full text-left px-5 py-2.5 font-bold uppercase tracking-wider text-[11px] text-muted-foreground hover:text-primary hover:bg-muted transition-colors">
                            {sub.label}
                          </button>
                        ))}
                      </div>
                  ...
                  <button key={item.label} onClick={() => handleMenuClick(item.href)} className="px-3 py-2 font-black rounded-xl transition-all whitespace-nowrap uppercase tracking-widest active:scale-95 text-[10px] text-muted-foreground hover:text-primary hover:bg-muted">
                    {item.label}
                  </button>
                )
              )}
            ...
            <div className="flex lg:hidden items-center justify-between w-full">
              <button className="p-2 rounded-xl transition-colors text-primary hover:bg-muted" onClick={() => setMobileMenuOpen(true)}>
                    ...
                    <div key={item.label}>
                      ...
                      <div className={`overflow-hidden transition-all duration-200 ${expandedMobileSection === item.label ? "max-h-80 opacity-100 mt-1" : "max-h-0 opacity-0"}`}>
                        <div className="pl-14 pr-3 space-y-0.5 pb-1">
                          {item.subItems.map((sub) => (
                            <button key={sub.label} onClick={() => handleMenuClick(sub.href)} className="w-full text-left px-4 py-2.5 rounded-xl text-[11px] font-bold text-muted-foreground hover:text-primary hover:bg-muted transition-colors uppercase tracking-wider">
                              {sub.label}
                            </button>
                          ))}
                        </div>
