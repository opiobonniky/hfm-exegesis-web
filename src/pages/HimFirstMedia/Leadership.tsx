      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 lg:px-12 bg-white">
        <div className="w-full max-w-4xl mx-auto">
          ...
          <motion.div variants={animFadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid sm:grid-cols-2 gap-8">
            {leadership.map((l) => (
              <div key={l.name} className="bg-brand-bg rounded-[2rem] p-6 sm:p-8 border border-slate-100 text-center">
          ...
          <motion.div variants={animFadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="mt-12 bg-brand-bg rounded-[2rem] p-8 sm:p-12 border border-slate-100 text-center">
            <p className="text-lg sm:text-xl text-slate-600 leading-relaxed font-medium italic">
            ...
            <p className="text-sm font-black text-slate-400 uppercase tracking-widest mt-4">— {t.himFirstMedia?.leadershipClosingRef || "Ephesians 4:11-12"}</p>
          </motion.div>
        </div>
      </section>
    </div>
