        ...
        <div className="w-full max-w-4xl mx-auto text-center">
          <motion.div variants={animFadeUp} initial="hidden" animate="visible">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 border border-white/20 mb-6">
              ...
              <span className="text-[10px] sm:text-xs font-black text-white/70 uppercase tracking-widest">Our Goals</span>
            </div>
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-black text-white font-[family-name:var(--font-heading)] tracking-tighter leading-none mb-6">
              Our <span className="text-brand-accent">Goals</span>
            </h1>
            <p className="text-lg sm:text-xl text-white/80 max-w-2xl mx-auto leading-relaxed font-medium">
              Clear targets we're pursuing to fulfill our calling and maximize Kingdom impact.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 lg:px-12 bg-white">
        <div className="w-full max-w-4xl mx-auto">
          <motion.div variants={animFadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="space-y-8">
            {[
              { icon: BookOpen, title: "Deepen Scriptural Engagement", desc: "Help users read, understand, and apply the Bible daily through multi-translation access, verse-by-verse teaching, and interactive reading plans." },
              { icon: Users, title: "Build a Global Prayer Community", desc: "Connect believers from every nation to pray for one another, share testimonies, and encourage each other in faith." },
              { icon: Globe, title: "Expand Language Reach", desc: "Make the platform accessible in 23+ languages so that the Word of God can reach every tongue and tribe." },
              { icon: Trophy, title: "Equip the Next Generation", desc: "Provide tools like journaling, reading challenges, and trivia that help young believers grow in their knowledge of Scripture." },
            ].map((g) => (
          ...
          <motion.div variants={animFadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="mt-12 text-center">
            <Link to="/register">
              <Button className="bg-brand-primary text-white hover:bg-brand-primary-dark px-8 py-6 rounded-[2rem] font-black text-base shadow-2xl shadow-brand-primary/20 uppercase tracking-widest">
                Join Us in Reaching These Goals <ArrowRight className="ml-2 w-5 h-5" />
              </Button>
            </Link>
          </motion.div>
        </div>
