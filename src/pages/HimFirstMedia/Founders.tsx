            ...
            {founders.map((f, i) => (
              <div key={f.name} className={`flex flex-col ${i % 2 === 1 ? "sm:flex-row-reverse" : "sm:flex-row"} gap-8 sm:gap-12 items-center`}>
                ...
                <div>
                  ...
                  <p className="text-base sm:text-lg text-slate-600 leading-relaxed font-medium">{f.bio}</p>
                </div>
              </div>
            ))}
          </motion.div>

          <motion.div variants={animFadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="mt-16 bg-brand-bg rounded-[2rem] p-8 sm:p-12 border border-slate-100 text-center">
            <Quote className="w-8 h-8 text-brand-accent mx-auto mb-4 opacity-50" />
            <p className="text-lg sm:text-xl text-slate-600 leading-relaxed font-medium italic mb-4">
