...
function getInitialFontSize(): number {
  try {
    const stored = Number.parseInt(localStorage.getItem("bible-font-size") || "16", 10);
...
export function useBibleReaderPage() {
  ...
  const updateFontSize = useCallback((size: number) => {
    ...
    try { localStorage.setItem("bible-font-size", String(validSize)); } catch { /* unavailable */ }
  }, []);
  useLayoutEffect(() => {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => { if (entry.isIntersecting) void reader.loadMore(); },
  ...
  const canGoNext = currentBookIndex < BIBLE_BOOKS.length - 1 || reader.selectedChapter < currentBook.maxChapter;
  ...
  const handleNextChapter = useCallback(() => {
    if (reader.selectedChapter < currentBook.maxChapter) {
      const chapter = reader.selectedChapter + 1;
  ...
  }, [currentBook.maxChapter, currentBookIndex, reader, scrollToChapter]);
