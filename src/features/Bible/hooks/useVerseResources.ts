...
export function useVerseResources() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try { setData(await getVerseResources(bookName, chapter, verseNumber)); }
    catch { setError("Failed to load verse resources."); }
    finally { setLoading(false); }
    setTranslationsLoading(true);
    try { setTranslations(await getTranslationComparison(bookName, chapter, verseNumber)); }
    catch { setTranslationsError("No translations available"); setTranslations(null); }
    finally { setTranslationsLoading(false); }
    setPrologueLoading(true);
    try { setPrologue(await getBookPrologue(bookName)); }
    catch { setPrologue(null); }
    finally { setPrologueLoading(false); }
  ...
  useEffect(() => { if (visibleTabs.length > 0 && !visibleTabs.includes(activeTab)) setActiveTab(visibleTabs[0]); }, [visibleTabs, activeTab]);
  useEffect(() => { setActiveTab("commentaries"); }, [bookName, chapter, verseNumber]);
  const goToReader = useCallback(() => {
    navigate(`/bible-reader?book=${encodeURIComponent(bookName)}&chapter=${chapter}&verse=${verseNumber}`);
  }, [navigate, bookName, chapter, verseNumber]);
  return {
    bookName, chapter, verseNumber, verseRef,
    activeTab, setActiveTab, data, loading, error,
    translations, translationsLoading, translationsError,
    prologue, prologueLoading, visibleTabs,
    goToReader,
  };
}
