...
export default function SearchPage() {
  ...
  const renderItem = (item: SearchResultItem, idx: number) => {
    ...
    return <SearchResultCard key={`bible-${idx}`} ref_={`${bi.book_name} ${bi.chapter}:${(bi as SearchResult).verse ?? (bi as CrossTranslationResult).verse}`}
      headline={(bi as any).headline} verseText={bi.verse_text || (bi as CrossTranslationResult).verse_text}
      translationAbbr={(bi as CrossTranslationResult).translationAbbr}
      onOpen={() => h.handleSelect(bi as SearchResult)} onStudy={() => h.handleStudy(bi as SearchResult)} onSave={() => h.handleSave(bi as SearchResult)} />;
  };
  return (
    <div className="min-h-screen flex flex-col bg-background" dir={h.isRtl ? "rtl" : "ltr"}>
          ...
          {h.scope === "bible" && (
            <div className="flex items-center gap-2">
              ...
              <Select value={h.translation} onValueChange={h.setTranslation}>
                ...
                <SelectContent>{h.CROSS_TRANSLATION_OPTIONS.map((tr) => (
                  <SelectItem key={tr.id} value={tr.id} className="text-xs py-1.5"><span className="font-semibold">{tr.abbr}</span> <span className="text-muted-foreground/60 text-[10px]">{tr.name}</span></SelectItem>
                ))}</SelectContent>
              </Select>
            </div>
          )}
      ...
      <div className="flex-1 overflow-hidden">
        <ScrollArea className="h-full">
          {h.scopeLocked && <div className="px-4 sm:px-6 py-4"><LockedFeatureBadge featureName="Advanced Search" featureDescription="Upgrade to access advanced search tools." /></div>}
          {!h.scopeLocked && <>
            {h.hasQuery && h.total > 0 && <div className="px-4 sm:px-6 py-2 mb-2"><p className="text-xs font-semibold text-muted-foreground">{h.total} result{h.total !== 1 ? "s" : ""}</p></div>}
            {h.showSkeleton ? <SearchSkeleton /> : h.hasQuery ? (
              <div className="px-4 sm:px-6 pb-6 space-y-3">
                ...
                {h.loading && h.results.length > 0 && <div className="flex items-center justify-center gap-2 py-4"><Loader2 className="w-4 h-4 animate-spin text-primary" /><span className="text-xs text-muted-foreground">Searching...</span></div>}
              </div>
            ) : (
            ...
            {!h.hasQuery && h.query.trim().length > 0 && h.query.trim().length < 3 && (
              <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
                <Search className="w-10 h-10 text-muted-foreground/20 mb-3" /><p className="text-sm text-muted-foreground">Type at least 3 characters to search</p>
            ...
            {h.searchError && <div className="flex flex-col items-center justify-center py-16 px-6 text-center"><p className="text-sm text-destructive">{h.searchError}</p></div>}
          </>}
          <div className="h-12" />
        </ScrollArea>
      </div>
