...
export default function ReadingSettingsPage() {
  ...
  return (
    <div className="space-y-6 p-6 max-w-2xl mx-auto">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Type className="h-5 w-5 text-primary" />Font Size</CardTitle>
          <CardDescription>Adjust the text size for reading</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            {FONT_SIZES.map((fs) => (
              <button key={fs.value} onClick={() => h.setFontSize(fs.value)}
          ...
          <div className="rounded-lg bg-muted/50 p-4">
            <p style={{ fontSize: h.fontSizePx }}>For God so loved the world that he gave his one and only Son, that whoever believes in him shall not perish but have eternal life. — John 3:16</p>
        </CardContent>
      </Card>
          <CardTitle className="flex items-center gap-2"><BookOpen className="h-5 w-5 text-primary" />Display Options</CardTitle>
          <div className="flex items-center justify-between">
            <div><p className="font-medium">Show Verse Numbers</p><p className="text-sm text-muted-foreground">Display verse numbers alongside text</p></div>
            <Switch checked={h.showVerseNumbers} onCheckedChange={h.setShowVerseNumbers} />
            <div><p className="font-medium">Auto-play Audio</p><p className="text-sm text-muted-foreground">Automatically play audio when opening a chapter</p></div>
        ...
        <CardContent>
            {[{ value: "light" as const, label: "Light", icon: Sun }, { value: "dark" as const, label: "Dark", icon: Moon }, { value: "system" as const, label: "System", icon: Monitor }].map(({ value, label, icon: Icon }) => (
              <button key={value} onClick={() => h.setTheme(value)}
                  h.theme === value ? "border-primary bg-primary/5" : "hover:border-primary/50")}>
                <Icon className="h-5 w-5" /><span className="font-medium text-sm">{label}</span>
    </div>
  );
}
