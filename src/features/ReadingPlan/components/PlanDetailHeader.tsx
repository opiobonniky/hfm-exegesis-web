...
import { GlassCard, SectionLabel, Ring, StatusBadge } from "./PlanDetailUI";
...
interface PlanDetailHeaderProps {
  ...
  lang: string;
  activeTab: string;
  setActiveTab: (tab: "overview" | "schedule" | "quiz" | "admin") => void;
  navigate: any;
  t: any;
  pct: number;
...
export function PlanDetailHeader({ plan, isAdmin, isRtl, lang, activeTab, setActiveTab, navigate, t, pct, displayQuizAccuracy, displayAnsweredQuestions, displayCorrectAnswers, displayWrongAnswers, totalReflections, configuredDays, configuredPct, totalQuizCount }: PlanDetailHeaderProps) {
  const diffKey = normalizeDifficulty(plan.difficulty);
  const diff = DIFFICULTY_CONFIG[diffKey] ?? DIFFICULTY_CONFIG.medium;
  const diffLabels: Record<string, string> = { easy: t.readingPlan.diffBeginner, medium: t.readingPlan.diffIntermediate, hard: t.readingPlan.diffAdvanced };
  const diffLabel = diffLabels[diffKey] || diffKey;
  const catKey = normalizeCategory(plan.category);
  const catLabels: Record<string, string> = { intro: t.readingPlan.catIntro, "whole-bible": t.readingPlan.catWholeBible, nt: t.readingPlan.catNT, ot: t.readingPlan.catOT, book: t.readingPlan.catBookByBook, topical: t.readingPlan.catTopical };
    ...
    <>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="flex flex-col gap-4">
          <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-muted-foreground hover:text-foreground transition-colors text-sm group w-fit">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border border-border bg-card text-muted-foreground">
              ...
              <Eye className="w-3 h-3" />{t.readingPlan.readOnly}
            <button onClick={() => navigate(routes.editReadingPlan.path.replace(":planId", plan.plan_id))} className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl border border-violet-200 bg-violet-50 text-violet-700 text-sm font-semibold hover:bg-violet-100 transition-colors">
              <Pencil className="w-4 h-4" />{t.readingPlan.editPlan}
            </button>
            {plan.started && !isAdmin && (
              <button onClick={() => { const nextDay = plan.completed_days_count + 1; navigate(`/daily-reading?planId=${plan.plan_id}&day=${Math.min(nextDay, plan.total_days)}`); }} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 text-white text-sm font-semibold hover:from-violet-700 hover:to-indigo-700 transition-all shadow-md shadow-violet-600/20">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            ...
            <GlassCard key={label} className="p-3 text-center">
              <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center mx-auto mb-2", accent)}><Icon className={cn("w-4 h-4", iconColor)} /></div>
        ...
        <GlassCard className="p-5">
          <div className="flex flex-col sm:flex-row items-center gap-6">
            ...
            <div className="flex-1 space-y-2 w-full">
              <div className="flex justify-between text-xs"><span className="text-muted-foreground">{t.readingPlan.quizAccuracy}</span><span className="font-semibold text-foreground">{displayQuizAccuracy}%</span></div>
              <div className="flex justify-between text-xs"><span className="text-muted-foreground">{isAdmin ? t.readingPlan.totalAnswers : t.readingPlan.answeredQuestions}</span><span className="font-semibold text-foreground">{displayAnsweredQuestions}</span></div>
              <div className="flex justify-between text-xs"><span className="text-muted-foreground">{t.readingPlan.correct}</span><span className="font-semibold text-emerald-700">{displayCorrectAnswers}</span></div>
              <div className="flex justify-between text-xs"><span className="text-muted-foreground">{t.readingPlan.wrong}</span><span className="font-semibold text-rose-700">{displayWrongAnswers}</span></div>
        </GlassCard>
        {/* Tab bar */}
        <div className="flex items-center gap-1 p-1 rounded-2xl border border-border bg-card w-fit shadow-sm">
          {(["overview", "schedule", "quiz", "admin"] as const).map((tab) => {
            ...
            return (
              <button key={tab} onClick={() => setActiveTab(tab)} className={cn("px-4 py-2 rounded-xl text-sm font-semibold capitalize transition-all duration-150", activeTab === tab ? "bg-violet-600 text-white shadow-sm" : "text-muted-foreground hover:text-foreground")}>
                {tab === "quiz" ? t.readingPlan.quizQuestionsLabel.replace("{n}", String(totalQuizCount)) : tab === "schedule" ? `${t.readingPlan.dailyAssignments} (${configuredDays}d)` : tab === "admin" ? t.readingPlan.adminInsights : t.readingPlan.overview}
            );
          })}
    </>
  );
