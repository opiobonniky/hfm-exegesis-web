              ...
              return (
                <div key={qi} className="rounded-xl border border-border bg-background p-3 space-y-2.5">
                  <div className="flex gap-2.5 items-start">
                    <span className="w-5 h-5 rounded-full border border-amber-200 bg-amber-50 text-[10px] font-bold text-amber-700 flex items-center justify-center shrink-0 mt-0.5">{idx}</span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 pl-7">
                    {q.options.map((opt, oi) => (
                      <div key={oi} className={cn("flex items-center gap-2 px-2.5 py-1.5 rounded-lg border text-xs", oi === q.correctAnswer ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-border bg-card text-muted-foreground")}>
                        {oi === q.correctAnswer ? <CheckCircle2 className="w-3 h-3 shrink-0" /> : <div className="w-3 h-3 rounded-full border border-border shrink-0" />}
                        {opt}
                      </div>
                    ))}
                  {q.explanation && (
