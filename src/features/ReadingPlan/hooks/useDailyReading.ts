...
export function useDailyReading() {
  ...
  useEffect(() => {
    if (selectedPlan) loadAssignments(selectedPlan.plan_id || selectedPlan.planId);
  }, [selectedPlan, loadAssignments]);

  const markComplete = useCallback(async () => {
    if (!selectedPlan) return;
    setCompleting(true);
    try {
      const res = await sendPostRequest("reading-plans", "complete-day", {
        planId: selectedPlan.plan_id || selectedPlan.planId,
