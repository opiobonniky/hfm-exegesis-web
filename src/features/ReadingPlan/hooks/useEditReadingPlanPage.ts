 ...
 * Extracts ALL useState/useEffect from EditReadingPlan.tsx.
 */
...
// Re-use types from existing types file
interface PlanMeta {
...
export function useEditReadingPlanPage() {
  ...
  const handleSaveMeta = useCallback(async () => {
    ...
    setSavingMeta(true);
    try {
      const res = await sendPostRequest("reading-plans", "update-plan-meta", { planId, title: meta.title, description: meta.description, isPublished: meta.isPublished });
  ...
  const handleSaveDay = useCallback(async (dayIdx: number) => {
    ...
    setSavingDay(dayIdx);
      ...
      const res = await sendPostRequest("reading-plans", "update-day", { planId, dayNumber: day.dayNumber, title: day.title, chapters: day.chapters, reflectionPrompt: day.reflectionPrompt, reflectionText: day.reflectionText, quizQuestions: day.quizQuestions });
  ...
  const handleDeleteQuiz = useCallback(async () => {
    ...
    setDeletingQuiz(true);
      ...
      const res = await sendPostRequest("reading-plans", "update-day", { planId, dayNumber: day.dayNumber, quizQuestions: newQuiz });
      if (res?.returnCode === 200) {
        setDays((prev) => prev.map((d, i) => i === dayIdx ? { ...d, quizQuestions: newQuiz } : d));
        toast({ title: "Quiz removed" });
      }
    } catch { toast({ title: "Delete failed", variant: "destructive" }); }
    finally { setDeletingQuiz(false); setDeleteQuizTarget(null); }
  }, [deleteQuizTarget, planId, days, toast]);
  const updateMeta = useCallback((field: string, value: any) => setMeta((prev) => prev ? { ...prev, [field]: value } : prev), []);
  const updateDay = useCallback((dayIdx: number, field: string, value: any) => setDays((prev) => prev.map((d, i) => i === dayIdx ? { ...d, [field]: value } : d)), []);
  const updateChapter = useCallback((dayIdx: number, chIdx: number, field: string, value: any) => {
    setDays((prev) => prev.map((d, i) => i === dayIdx ? { ...d, chapters: d.chapters.map((c, j) => j === chIdx ? { ...c, [field]: value } : c) } : d));
  }, []);
  return {
