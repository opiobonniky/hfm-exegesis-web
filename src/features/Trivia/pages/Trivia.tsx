  ...
  return (
    <>
      ...
      >
        ...
        />
        <StarBurst
          visible={p.showStarBurst}
          onFinish={() => p.setShowStarBurst(false)}
        />
        <SanctuarySeal
          visible={p.showMilestone}
          total={p.score.total}
          correct={p.score.correct}
          percentage={
            p.score.total > 0
              ? Math.round((p.score.correct / p.score.total) * 100)
              : 0
          }
          onFinish={() => p.setShowMilestone(false)}
        />
        <TriviaContentWrapper>
          {p.gameMode === "daily" && (
            <DailyChallengeGame
              ...
              todayKey={p.todayKey}
              onSelect={p.handleSelectDaily}
              onDismiss={p.handleDismissDaily}
              onBack={p.handleDailyBackToPlan}
              onStart={p.startDailyChallenge}
              onReferencePress={p.handleReferencePress}
            />
          )}
          {p.gameMode !== "daily" && p.phase === "plan" && (
            <TriviaPlanScreen
              difficulty={p.difficulty}
              setDifficulty={p.setDifficulty}
              stats={p.stats}
              isTodayCompleted={p.isTodayCompleted}
              consecutiveDays={p.consecutiveDays}
              todayKey={p.todayKey}
              weekHistory={p.weekHistory}
              startDailyChallenge={p.startDailyChallenge}
              startQuiz={p.startQuiz}
              leaderboardState={p.leaderboardState}
              leaderboardComparison={p.leaderboardComparison}
              resetLeaderboard={p.resetLeaderboard}
              isRtl={p.isRtl}
            />
          )}
          {p.gameMode !== "daily" && p.phase !== "plan" && (
            <TriviaGameScreen
              ...
              loading={p.loading}
              error={p.error}
              resultDismissed={p.resultDismissed}
              autoAdvanceProgress={p.autoAdvanceProgress}
              isRtl={p.isRtl}
              onSelect={p.handleSelect}
              onDismiss={p.handleDismissWithCancel}
              onNext={p.nextQuestion}
              onReset={p.reset}
              onSetDifficulty={p.setDifficulty}
              onReferencePress={p.handleReferencePress}
              leaderboardComparison={p.leaderboardComparison}
              leaderboardState={p.leaderboardState}
              stats={p.stats}
