    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6" dir={h.isRtl ? "rtl" : "ltr"}>
      ...
      <Dialog open={!!h.deleteTarget} onOpenChange={o => !o && h.setDeleteTarget(null)}>
        <DialogContent className="sm:max-w-md rounded-xl">
          ...
          {h.deleteTarget && (
            <div className="py-2">
              <p className="text-sm font-medium mb-1">{h.deleteTarget.question}</p>
              <p className="text-xs text-muted-foreground">Difficulty: {h.deleteTarget.difficulty} · Category: {h.deleteTarget.category}</p>
            </div>
          )}
