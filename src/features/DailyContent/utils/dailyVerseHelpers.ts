...
export const getConflictMessage = (conflict: any, t?: any): string => {
  if (!conflict) return "";
  const ref =
    conflict.existing?.bookName +
    " " +
    conflict.existing?.chapter +
    ":" +
    conflict.existing?.verseNumber;
  const date = conflict.existing?.displayDate || "";
  const dv = t?.dailyVerse;
  if (conflict.type === "date") {
    const msg =
      dv?.verseConflictForDate ||
      "A verse already exists for this date ({ref}).";
    return msg.replace("{ref}", ref);
  const msg =
    dv?.verseConflictForVerse ||
    "This verse ({ref}) already exists for {date}.";
