// Dev-only: crawl user-facing pages (Home dashboard + Bible) in DARK mode and flag:
//  - pages that don't go dark (html.dark missing)
//  - large light surfaces (opaque near-white slabs > 20% of viewport)
//  - poor text contrast on those slabs
// Requires backend :5001 + vite :8080. Usage: node verify-user-dark.mjs <jwt>
import { launch } from "puppeteer-core";
import { execSync } from "node:child_process";

const token = process.argv[2];
if (!token) {
  console.error("usage: node verify-user-dark.mjs <jwt>");
  process.exit(2);
}
...
await page.screenshot({ path: "/tmp/user-dark-crawl.png" });
const clean = results.filter((r) => !r.error && r.htmlDark && !r.issues.length);
console.log(
  `\n=== ${clean.length}/${PAGES.length} pages dark-clean; pageErrors: ${pageErrors.length}`,
);
